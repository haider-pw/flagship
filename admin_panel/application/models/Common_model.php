...
// query returned no results
    ...
    public function update_batch($tbl, $data, $where){
        $this->db->update_batch($tbl, $data, $where);
        $affectedRows = $this->db->affected_rows();
        if ($affectedRows) {
            return true;
        } else {
    ...
    public function count_group_by($tbl, $count_id, $group_by){
        $this->db->select('country, COUNT("'.$count_id.'") as total');
        $this->db->group_by($group_by);
        $query=$this->db->get($tbl);
        return $query->result();
    }
    // where in
      ...
      function select_fields_where_in_like_join($tbl = '', $data, $joins = '', $where = '',$where_in = array(), $single = FALSE, $field = '', $value = '',$group_by='',$order_by = '',$limit = '',$array = false)
    {
        if (is_array($data) and isset($data[1])){
            $this->db->select($data[0],$data[1]);
        }else{
