...
$room_type = mysql_fetch_row(mysql_query("SELECT * FROM bgi_roomtypes WHERE id_room='" . QuoteSmart($_GET['id']) . "'"));
$room_id = $_GET['id'];
        ...
        if(! $retval )
            {
				...
				echo "<script>window.location='location-details.php?id=".$location_id."&ok=3'</script>";
        mysql_close($conn);
	}
ob_end_flush();
?>
