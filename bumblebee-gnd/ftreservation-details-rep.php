...
$get_rep = mysql_fetch_row(mysql_query("SELECT * FROM gnd_reps WHERE id_rep='" . $reservation[42] . "'"));
$repselect = mysql_query("SELECT * FROM gnd_reps ORDER BY name ASC");
...
//Sanitize data
    ...
    $rep_action = mysql_fetch_row(mysql_query("SELECT * FROM gnd_reps WHERE id_rep='" . $rep_name . "'"));
 ...
 //Put all this stuff into the database
    ...
    $sql_4 = "INSERT INTO gnd_activity ". 
        ...
        mysql_close($conn);
        $ok= $_GET['id'];
        header('Location:ftreservation-details-rep.php?id=' . $ok . '');
	}
    ob_end_flush();
?>
