...
$reservation = mysql_fetch_row(mysql_query("SELECT * FROM gnd_reservations WHERE id='" . QuoteSmart($_GET['id']) . "'"));
$get_arr_flight_no = mysql_fetch_row(mysql_query("SELECT * FROM gnd_flights WHERE id_flight='" . $reservation[16] . "'"));
$get_arr_time = mysql_fetch_row(mysql_query("SELECT * FROM gnd_flighttime WHERE id_fltime='" . $reservation[15] . "'"));
$get_dpt_flight_no = mysql_fetch_row(mysql_query("SELECT * FROM gnd_flights WHERE id_flight='" . $reservation[28] . "'"));
$get_dpt_time = mysql_fetch_row(mysql_query("SELECT * FROM gnd_flighttime WHERE id_fltime='" . $reservation[27] . "'"));
$get_arr_pickup = mysql_fetch_row(mysql_query("SELECT * FROM gnd_location WHERE id_location='" . $reservation[21] . "'"));  
$get_arr_location = mysql_fetch_row(mysql_query("SELECT * FROM gnd_location WHERE id_location='" . $reservation[22] . "'"));
$get_arr_roomtype = mysql_fetch_row(mysql_query("SELECT * FROM gnd_roomtypes WHERE id_room='" . $reservation[23] . "'"));  
$get_arr_driver = mysql_fetch_row(mysql_query("SELECT * FROM gnd_transport WHERE id_transport='" . $reservation[19] . "'"));
$get_arr_vehicle = mysql_fetch_row(mysql_query("SELECT * FROM gnd_vehicles WHERE id_vehicle='" . $reservation[20] ."'"));
$get_dpt_driver = mysql_fetch_row(mysql_query("SELECT * FROM gnd_transport WHERE id_transport='" . $reservation[30] . "'"));
$get_dpt_vehicle = mysql_fetch_row(mysql_query("SELECT * FROM gnd_vehicles WHERE id_vehicle='" . $reservation[31] . "'"));
$get_touroperator = mysql_fetch_row(mysql_query("SELECT * FROM gnd_touroperator WHERE id='" . $reservation[5] . "'"));
$get_dptpickup = mysql_fetch_row(mysql_query("SELECT * FROM gnd_location WHERE id_location='" . $reservation[32] . "'"));
$get_dptdropoff = mysql_fetch_row(mysql_query("SELECT * FROM gnd_location WHERE id_location='" . $reservation[33] . "'"));
$flagship_ref = $reservation[40];


//Get and count how many guests are on this reservation
$guestrows = mysql_query("SELECT * FROM gnd_guest WHERE ref_no_sys='$flagship_ref'");
$guest_count = mysql_num_rows($guestrows);

$transport_arr = mysql_query("SELECT * FROM gnd_resdrivers WHERE ref_no_sys='$flagship_ref' AND res_type=1");
$arr_count = mysql_numrows($transport_arr);

$transport_dpt = mysql_query("SELECT * FROM gnd_resdrivers WHERE ref_no_sys='$flagship_ref' AND res_type=2");
$dpt_count = mysql_numrows($transport_dpt);

//Get tour operator list
$operselect = mysql_query("SELECT * FROM gnd_touroperator ORDER BY tour_operator ASC");
$dptlocationselect = mysql_query("SELECT * FROM gnd_location ORDER BY name ASC");
...
//Sanitize data
    ...
    if ($arr_driver > 0 && $arr_count > 0){
        ...
        $res_type_arr = 1;
        $sql2 = "INSERT INTO gnd_resdrivers ". 
    ...
    if ($dpt_driver > 0 && $dpt_count > 0){
        ...
        $res_type_dpt = 2;
        $sql3 = "INSERT INTO gnd_resdrivers ". 
        "(transport, vehicle, ref_no_sys, adult, child, infant, tour_operator, location, pickup_time,  comments, flight_time, flight_no, infant_seats, child_seats, booster_seats, title_name, first_name, last_name, transport_date, res_type) ". 
